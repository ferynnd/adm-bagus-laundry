...
import dev.ferynnd.admbaguslaundry.data.helper.Constant
import dev.ferynnd.admbaguslaundry.data.helper.SharePrefrenceHelper
import dev.ferynnd.admbaguslaundry.data.viewmodel.NetworkViewModel
import dev.ferynnd.admbaguslaundry.data.viewmodel.UserViewModel
import dev.ferynnd.admbaguslaundry.databinding.ActivityLoginBinding
import dev.ferynnd.admbaguslaundry.model.UserRole
...
class LoginActivity : AppCompatActivity() {
        ...
        networkViewModel.isConnected.observe(this) { isConnected ->
            if (isConnected) {
                ...
                userViewModel.loginResult.observe(this) { result ->
                    result.onSuccess {
                        if (it.data != null) {
                            if (it.data.role_user == UserRole.admin) {
                                sharedPreferences.put(Constant.PREF_IS_LOGIN, true)

                                it.data.username?.let { it1 ->
                                    sharedPreferences.put(
                                        Constant.PREF_USER_NAME,
                                        it1
                                    )
                                }

                                it.token?.let { it1 ->
                                    sharedPreferences.put(Constant.PREF_USER_TOKEN, it1)
                                }

                                it.data.id_user.let { it1 ->
                                    sharedPreferences.put(Constant.PREF_USER_ID, it1.toString())
                                }

                                it.data.role_user.let { it1 ->
                                    sharedPreferences.put(Constant.PREF_USER_ROLE, it1.toString())
                                }
                                ...
                                Alerter.create(this)
                                    ...
                                    .setText("Aplikasi ini hanya diperuntukkan untuk admin.")
                    ...
                    result.onFailure { throwable ->
                        if (throwable is ConnectException) {
                            ...
                            Toast.makeText(
                                this,
                                "Login Error Kesalahan Username atau Password",
                                Toast.LENGTH_SHORT
                            ).show()
    ...
    private fun checkLogin() {
        ...
        if (role == UserRole.admin.toString()) {
            ...
            sharedPreferences.clear()
            Toast.makeText(this, "Aplikasi ini hanya untuk admin", Toast.LENGTH_SHORT).show()
        }
    }
