...
class RentalReportAdapter(private val onDetail: (ReportRental) -> Unit) :
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when (holder) {
            is ReportRentalViewHolder -> {
                ...
                val branchName =
                    branches.find { it.id_branch == transactionReportRental.id_branch_transaction_rental }?.name_branch
                        ?: "Unknown"
                val senderName =
                    sender.find { it.id_user == transactionReportRental.id_kurir_transaction_rental }?.fullname_user
                        ?: "Unknown"
                val clientName =
                    client.find { it.id_client == transactionReportRental.id_client_transaction_rental }?.name_client
                        ?: "Unknown"
                holder.binding.apply {
                    cabangTransaksiRental.text = branchName ?: "-"
                    namaClientTransaksiRental.text = clientName ?: "-"
                    namaKurirTransaksiRental.text = senderName ?: "-"
                    namaKurirTransaksiRental.text = transactionReportRental.recipient_name_transaction_rental ?: "-"
                    totalBeratTransaksiRental.text =
                        transactionReportRental.total_weight_transaction_rental.toString() + " Kg" ?: "-"
                    jumlahItemTransaksiRental.text = transactionReportRental.total_pcs_transaction_rental.toString() + " Pcs" ?: "-"
                    noteTransaksiRental.text = transactionReportRental.notes_transaction_rental ?: "-"

                    tanggalTransaksiRental.text = transactionReportRental.time_transaction_rental ?: "-"

                    noResiTransaksiRental.text =
                        transactionReportRental.id_transaction_rental.toString() ?: "-"
            ...
            is HeaderViewHolder -> {
                ...
                val color = ContextCompat.getColor(
                    context,
                    R.color.blueGray
                ) // pastikan 'orange' benar ada di colors.xml
